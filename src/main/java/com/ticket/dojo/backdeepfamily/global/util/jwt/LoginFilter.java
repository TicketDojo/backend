...
public class LoginFilter extends UsernamePasswordAuthenticationFilter{
     ...
     *
     * @param request HTTP 요청 (username, password 파라미터 포함)
     * @param response HTTP 응답
     * @return Authentication 인증 결과 객체
     *
     * 동작 과정:
     * 1. 요청에서 username(실제로는 email)과 password 추출
     ...
     *
     * 예시:
     * - 요청: POST /login (username=user@example.com, password=1234)
     * - DB 조회: user@example.com의 정보와 비밀번호 비교
     * - 결과: 인증 성공 → Authentication 객체 반환
     */
    @Override
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
            throws AuthenticationException {

        // 요청에서 username 파라미터 추출 (실제로는 email 값)
        String username = obtainUsername(request);

        // 요청에서 password 파라미터 추출
        String password = obtainPassword(request);

        // 인증 전 토큰 생성 (아직 인증되지 않은 상태)
        // 파라미터: username, password, 권한(null - 아직 모름)
        UsernamePasswordAuthenticationToken authToken =
            new UsernamePasswordAuthenticationToken(username, password, null);

        // AuthenticationManager에게 인증 요청
        // 내부적으로 CustomUserDetailsService.loadUserByUsername()을 호출하여
        // DB에서 사용자 정보를 조회하고 비밀번호를 BCrypt로 검증
     ...
     * - 출력: Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6...
